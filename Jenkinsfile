...
node(isProtectedBranch() ? 'BlazorSample' : 'BlazorSamples') {
    timeout(time:180){
        try {
            ...
            stage('Checkout') {
                checkout([$class: 'GitSCM', branches: [[name: '*/$githubSourceBranch']], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: ''],[$class: 'CloneOption', noTags: true, shallow: true, depth: 1, timeout: 30]], submoduleCfg: [], userRemoteConfigs: [[credentialsId: env.GiteaCredentialID, url: 'https://gitea.syncfusion.com/essential-studio/blazor-document-processing-samples.git']]])
            }
            ...
            if(blazor.checkCommitMessage()) {
                ...
                stage('Build') {
                    ...
                    blazor.runShell('npm run build-documentSDK-net9');
                    blazor.runShell('npm run build-documentSDK-net8');
                }
                ...
                stage('Publish') {
                    if(blazor.isProtectedBranch()) {
                        ...
                        blazor.runShell('npm run publish-documentSDK-net9');
                        blazor.runShell('npm run publish-documentSDK-net8');
                    }
                }

                stage('Deploy') {
                    if(blazor.isProtectedBranch()) {
                        blazor.runShell('gulp generate-sitemap --option server --option local-sitemap');
                        blazor.runShell('gulp generate-sitemap --option wasm --option local-sitemap');
                        blazor.runShell('npm run blazor-deploy-documentSDK'); 
