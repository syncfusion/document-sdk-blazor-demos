    ...
    public class DigitalSignatureValidationPdfService
    {
        ...
        public string DigitalSignatureValidation()
        {
            ...
            if (lSigFld == null)
            {
                ldoc.Close(true);
                Close();
                return "No signature field found in the document.";
            }
            ...
            //Signature details
            if (lSigFld.Signature?.Certificate != null)
            {
            ...
            if (result.SignerCertificates != null)
            {
                foreach (PdfSignerCertificate signerCertificate in result.SignerCertificates)
                {
                    if (signerCertificate.OcspCertificate != null)
                    {
                        ...
                        builder.AppendLine();
                        if (signerCertificate.OcspCertificate.Certificates != null)
                        {
                            foreach (X509Certificate2 item in signerCertificate.OcspCertificate.Certificates)
                            {
                                builder.AppendLine("The OCSP Response was signed by " + (item.SubjectName?.Name ?? "Unknown"));
                            }
                        }
                    ...
                    if (signerCertificate.CrlCertificate != null)
                    {
                        ...
                        builder.AppendLine();
                        if (signerCertificate.CrlCertificate.Certificates != null)
                        {
                            foreach (X509Certificate2 item in signerCertificate.CrlCertificate.Certificates)
                            {
                                builder.AppendLine("The CRL was signed by " + (item.SubjectName?.Name ?? "Unknown"));
                            }
                        }
