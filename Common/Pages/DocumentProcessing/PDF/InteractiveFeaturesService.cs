        ...
        {                      
            using (StreamReader reader = new StreamReader(fileDataValue["adventure-work-cycle.xml"], true))
            {
                return XElement.Parse(reader.ReadToEnd())
                    ...
                    {
                        ProductID = c.Element("ProductID")?.Value ?? string.Empty,
                        Product = c.Element("Product")?.Value ?? string.Empty,
                        Price = c.Element("Price")?.Value ?? string.Empty,
                        Quantity = c.Element("Quantity")?.Value ?? string.Empty,
                        TotalPrice = c.Element("TotalPrice")?.Value ?? string.Empty
                    });
            }
        }
        public void Close()
