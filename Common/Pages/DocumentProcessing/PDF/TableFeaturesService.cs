...
{ 
    ...
    {
        private readonly Dictionary<string, MemoryStream> fileDataValue = null!;
        public TableFeaturesService(Dictionary<string, MemoryStream> fileData)
        {
            fileDataValue = fileData;
        }
        #region fields
        PdfPen borderPen = null!;
        PdfPen transparentPen = null!;
        float cellSpacing = 7f;
        float margin = 40f;
        PdfFont smallFont = null!;
        #endregion
        ...
        void pdfLightTable_BeginCellLayout(object sender, BeginCellLayoutEventArgs args)
        {
            if (args.RowIndex > -1 && args.CellIndex > -1)
            {
                ...
                if (args.Value != null && args.Value.Contains("http"))
                {
                    args.Skip = true;
            ...
            public static IEnumerable<Products> GetProducts(string webRootPath)
            {
                ...
                using (StreamReader reader = new StreamReader(xmlStream, true))
                {
                    return XElement.Parse(reader.ReadToEnd())
                        ...
                        {
                            Image1 = c.Element("Image1")?.Value ?? string.Empty,
                            Description1 = c.Element("Description1")?.Value ?? string.Empty,
                            Image2 = c.Element("Image2")?.Value ?? string.Empty,
                            Description2 = c.Element("Description2")?.Value ?? string.Empty,
                            Image3 = c.Element("Image3")?.Value ?? "dummy",
                            Description3 = c.Element("Description3")?.Value ?? "dummy",
                        });
                }
            }
            ...
            public static IEnumerable<Report> GetReport(string webRootPath)
            {
                ...
                using (StreamReader reader = new StreamReader(xmlStream, true))
                {
                    return XElement.Parse(reader.ReadToEnd())
                        ...
                        .Select(c => new Report
                        {
                            Image = c.Element("Image")?.Value ?? string.Empty,
                            Description = c.Element("Description")?.Value ?? string.Empty,
                        });
                }
            }
        }
