...
namespace BlazorDemos.Data.DocumentProcessing.PowerPoint
{
    public class OLEObjectService
    {
        ...
        public MemoryStream? ExtractOleObject()
        {
            ...
            // Gets the Ole Object of the slide safely
            if (slide.Shapes[2] is IOleObject oleObject && oleObject.ObjectData != null)
            {
                // Save the document as a stream and return the stream
                return new MemoryStream(oleObject.ObjectData);
            }

            // Return null if oleObject is not found or ObjectData is null
            return null;
        }
