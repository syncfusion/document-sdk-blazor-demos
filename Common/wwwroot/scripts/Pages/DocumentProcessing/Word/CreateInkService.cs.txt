...
{
    public class CreateInkService
    {
        private readonly Dictionary<string, MemoryStream> fileDataValue;
        public CreateInkService(Dictionary<string, MemoryStream> fileData)
        {
            fileDataValue = fileData;
        }

        public MemoryStream CreateInk(string documentType, string button)
        {
            if (button == "View Template")
            {
                return fileDataValue["create-ink-input.docx"];
            }

            // Creates an empty Word document instance
            WordDocument document = new WordDocument();
            // Opens template document
            document.Open(fileDataValue["create-ink-input.docx"], FormatType.Docx);
            // Add a new paragraph to the second cell of the first row in the table
